...
class SamanantarEnglishHindiTranslationDataset(Dataset):
    def __init__(self, data: Dict[str, List[str]], tokenizer: MT5Tokenizer, source_max_length: int, target_max_length: int):
        ...
        self.source_texts = data["tgt"]
        self.target_texts = data["src"]
...
class SamanantarMarathiEnglishTranslationDataset(Dataset):
    def __init__(self, data: Dict[str, List[str]], tokenizer: MT5Tokenizer, source_max_length: int, target_max_length: int):
        self.tokenizer = tokenizer
        self.source_max_length = source_max_length
        self.target_max_length = target_max_length
...
def run(pretrained_model, device, train_dataset, val_dataset, num_epochs):
    ...
    for epoch in range(num_epochs):
        if not early_stopping.early_stop:
            ...
            early_stopping(val_bleu_score, model, f'./checkpoints/objective_one_back_translation/model_checkpointed')
...
def load_dataset(lang, tokenizer):
    ...
    if lang == "hindi":
        ...
        train_dataset_final = SamanantarEnglishHindiTranslationDataset(train_dataset, tokenizer, 128, 128)
        val_dataset_final = SamanantarEnglishHindiTranslationDataset(val_dataset, tokenizer, 128, 128)
    elif lang =="marathi":
        dataset = load_from_disk("./data/marathi_dataset_20k")
        dataset = dataset.train_test_split(test_size=0.2, seed=42)
        train_dataset = dataset["train"].shuffle(seed=42)
        val_dataset = dataset["test"].shuffle(seed=42)
        train_dataset_final = SamanantarMarathiEnglishTranslationDataset(train_dataset, tokenizer, 128, 128)
        val_dataset_final = SamanantarMarathiEnglishTranslationDataset(val_dataset, tokenizer, 128, 128)
    else:
...
def run_for_all_languages(pretrained_model, device, tokenizer, num_epochs):
    ...
    for index, lang in enumerate(languages_used):
        if index == 0:
            print("Starting Hindi to english")
            ...
            print("Starting Marathi to english..")
            train_dataset, val_dataset  = load_dataset(lang, tokenizer)
    ...
    with open('./data/pickles/objective_one_back_metrics.pickle', 'wb') as handle:
...
# Load the tokenizer and model
    ...
    wandb.init(
        ...
        project="objective1back_cs685_project", 
